...
type DocumentNodesDependencies = {
  ensureFlowsForInformation: (nodeId: string, infoIds: string[]) => void
}

export function createDocumentNodesUseCases(
  context: EditorDocumentContext,
  dependencies: DocumentNodesDependencies,
) {
  ...
  function deleteNode(nodeId: string): void {
    ...
    context.comments.value = context.comments.value.filter(comment => comment.targetId !== encodeTargetId('node', nodeId))
  }
